...
class Animal(object):
    ...
    def introduce(self):
        if (not isinstance(self.get_name(), str) or not self.get_name()) and (
                not isinstance(self.get_color(), str) or not self.get_color()):
            print('It\'s %s!!!' % self.__class__.__name__)
        elif not isinstance(self.get_name(), str) or not self.get_name():
            print(
                'It\'s %s %s!!!' %
                (self.get_color(), self.__class__.__name__))
        elif not isinstance(self.get_color(), str) or not self.get_color():
            print('%s is %s!!!' % (self.get_name(), self.__class__.__name__))
        else:
            print(
                '%s is %s %s!!!' %
                (self.get_name(),
